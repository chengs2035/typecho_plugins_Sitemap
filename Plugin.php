...
class Sitemap_Plugin implements Typecho_Plugin_Interface
{
    ...
    public static function config(Typecho_Widget_Helper_Form $form)
    {
        ...
        //增加不在地图中展示附件链接的操作
        $ignoreAttachment = new Typecho_Widget_Helper_Form_Element_Radio('ignoreAttachment', array('1'=>_t('是'), '0'=>_t('否')), 0, _t('跳过附件'), _t('不在地图中展示附件地图'));
        ...
        $form->addInput($ignoreAttachment);
    }
    ...
    public static function generater($content)
    {
        ...
        if(!is_writable($dir))
        {
            @chmod($dir, 0777);
            if(!is_writable($dir))
            {   throw new exception('['.$dir.']指定的目录不可写');    }
        }
        ...
        $ignoreAttachmentStr="1=1";
        $ignoreAttachmentStrValue="";
        //跳过附件地图
        if($config->ignoreAttachment=='1'){
            $ignoreAttachmentStr='table.contents.type <> ?';
            $ignoreAttachmentStrValue="attachment";
        }
        /* posts */
        $posts = $db->fetchAll($db->select()
            ->from('table.contents')
            ->where('table.contents.status = ?', 'publish')
            ->where($ignoreAttachmentStr,$ignoreAttachmentStrValue)
